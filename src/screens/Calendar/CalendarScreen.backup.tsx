...
export const CalendarScreen: React.FC<Props> = ({ navigation }) => {
      ...
      for (const calendar of calendars) {
        try {
          ...
          for (const nativeEvent of nativeEvents) {
            try {
              // Skip events that are very short (< 1 minute) or all-day events without proper times
  ...
  }, [calendarPermission, user, refresh]);
  ...
  const syncEventToNativeCalendar = useCallback(
    ...
    [calendarPermission, refresh, requestCalendarPermission]
  );
